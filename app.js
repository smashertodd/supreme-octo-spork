...
function onDrop(e, container) {
  e.preventDefault();
  container.classList.remove("drop-target-over");
  if (!dragSrcEl) return;
...
function onTouchEnd(e) {
  ...
  if (container) {
    const target = el.closest(".sentence-chip");
    if (target && target !== touchChip && target.parentNode === container) {
      container.insertBefore(touchChip, target);
    } else {
      container.appendChild(touchChip);
    }
  }
  touchChip = null;
}
...
function checkAnswer() {
  ...
  if (submitted.length === 0) {
    showFeedback("Drag the sentences into the box first!", "neutral");
    return;
  }
  ...
  if (orderCorrect) {
    partsInZone.forEach((chip, i) => {
      if (chip.textContent !== correctParts[i].text) orderCorrect = false;
    });
  }
  ...
  const partial = allPartsPresent && noDistractors && !orderCorrect;
  ...
  submitted.forEach(chip => {
    ...
    if (chip.dataset.distractor === "true") {
      ...
      const idx = partsInZone.indexOf(chip);
      chip.classList.add(idx >= 0 && chip.textContent === correctParts[idx]?.text ? "correct" : "incorrect");
    }
  });
  ...
  if (perfect) {
    status = "correct";
    message = "🎉 Perfect! You got the order exactly right and spotted the distractor(s).";
  } else if (noDistractors && !orderCorrect) {
    status = "partial";
    message = "✅ Good — you kept the distractors out! But check the order of your sentences.";
  } else if (!noDistractors && orderCorrect) {
    status = "partial";
    message = "⚠️ The order is right, but there's a sentence in there that doesn't belong — can you spot it?";
  } else {
    status = "incorrect";
    message = "❌ Not quite. Check which sentences belong and what order they go in.";
  }
  
  showFeedback(message, status);
  trackAttempt(status, submitted.map(c => c.textContent));
