...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("refresh-data-btn").addEventListener("click", updateTeacherDashboard);

    // Neon Toggle Button Logic
    const teacherBtn = document.getElementById("btn-teacher-toggle");
    teacherBtn.addEventListener("click", () => {
        if (!isTeacherView) {
            Swal.fire({
                ...
                background: '#1e1b4b', color: '#fff', confirmButtonColor: '#ec4899'
            }).then(result => {
                if (result.value === TEACHER_PIN) {
                    ...
                    teacherBtn.innerHTML = "← Back to Game";
                    teacherBtn.classList.replace('text-pink-300', 'text-white');
            ...
            teacherBtn.innerHTML = "Teacher Login 🔐";
            teacherBtn.classList.replace('text-white', 'text-pink-300');
        }
    });

    setInterval(() => {
        if (isTeacherView) updateTeacherDashboard();
    }, 15000);
});
    ...
    Papa.parse(LIBRARY_CSV_URL, {
        ...
        complete: function(results) {
            ...
            if (activities.length === 0) {
                document.getElementById("loading-screen").innerHTML = "<h2 class='text-white text-2xl font-bold'>No Active Games Found!</h2>";
                return;
            }
            document.getElementById("loading-screen").classList.add("hidden");
            document.getElementById("app-workspace").classList.remove("hidden");
...
function renderParagraphBuilderActivity(data, leftPanel, rightPanel) {
    ...
    data.forEach(row => {
        ...
        rowDiv.className = "flex w-full gap-4";
        
        let labelBox = document.createElement('div');
        labelBox.className = "w-1/3 flex items-center justify-between p-4 rounded-xl text-gray-900 font-bold shadow-md";
        labelBox.style.backgroundColor = COLORS[activeIndex % COLORS.length];
        labelBox.innerHTML = `<span>${row.Label || 'Part ' + (activeIndex + 1)}</span>`;
        if (row.Hint) labelBox.innerHTML += `<button onclick="Swal.fire('Hint', '${row.Hint.replace(/'/g, "\\'")}', 'info')" class="text-gray-900 hover:text-white transition-colors text-xl">💡</button>`;
...
function renderCategorisationActivity(data, leftPanel, rightPanel) {
    ...
    data.forEach((row, index) => {
        ...
        block.className = "text-block w-full bg-white/5 border border-white/10 rounded-xl p-5 flex items-center gap-4 transition-colors duration-500 shadow-lg";
        block.dataset.index = index;
        
        let hintHtml = row.Hint ? `<button onclick="Swal.fire('Hint', '${row.Hint.replace(/'/g, "\\'")}', 'info')" class="ml-auto text-yellow-300 hover:scale-110 transition-transform text-xl">💡</button>` : '';
        let innerHtml = '';
        const parts = row.Text.split('___');
        
        parts.forEach((part, i) => {
            innerHtml += `<span class="text-white text-xl">${part}</span>`;
            if (i < parts.length - 1) {
        ...
        block.innerHTML = `<div class="flex-1 flex items-center flex-wrap gap-1">${innerHtml}</div>${hintHtml}`;
...
function renderFlowingGapFillActivity(data, leftPanel, rightPanel) {
    ...
    wrapper.className = 'text-white text-2xl leading-[3] p-8 bg-white/5 rounded-2xl border border-white/10 shadow-xl w-full h-full';
        ...
        if (row.Text.includes('___')) {
            ...
            parts.forEach((part, i) => {
                ...
                if (i < parts.length - 1) {
                    ...
                    dropzone.className = "dropzone inline-flex items-center justify-center min-w-[120px] h-[45px] border-2 border-dashed border-white/40 rounded-full bg-black/20 mx-2 px-3 align-middle transition-all";
                    dropzone.dataset.expected = row.Label; dropzone.dataset.mode = 'flowing';
                    dropzone.addEventListener('dragover', e => { e.preventDefault(); dropzone.classList.add('border-pink-400', 'bg-pink-500/20'); });
                    dropzone.addEventListener('dragleave', () => dropzone.classList.remove('border-pink-400', 'bg-pink-500/20'));
                    dropzone.addEventListener('drop', e => handleDrop(e, dropzone));
...
function renderDetectiveActivity(data, leftPanel, rightPanel) {
    leftPanel.innerHTML = `<div class="bg-white/5 p-8 rounded-2xl border border-white/10 text-center shadow-xl h-full flex flex-col justify-center"><h3 class="text-3xl text-pink-300 font-bold mb-4">🔍 Detective Mode</h3><p class="text-white text-xl">Click the words in the text to highlight <strong>${data[0].Label}</strong>.</p></div>`;
    const textBox = document.createElement('div');
    textBox.className = "bg-white/5 border border-white/10 rounded-2xl p-8 text-2xl text-white leading-[2.5] shadow-xl w-full h-full";
    
    data.forEach((row) => {
        if (!row.Text) return;
        const p = document.createElement('p'); p.className = "mb-6";
        let processedText = row.Text.replace(/\[\[(.*?)\]\]/g, `<span class="detective-word cursor-pointer px-2 py-1 rounded-lg transition-all border border-transparent hover:border-pink-500/50" data-target="true">$1</span>`);
        ...
        if(row.Hint) p.innerHTML += `<button onclick="Swal.fire('Hint', '${row.Hint.replace(/'/g, "\\'")}', 'info')" class="ml-2 text-yellow-300 hover:scale-110 transition-transform">💡</button>`;
    ...
    document.querySelectorAll('.detective-word').forEach(word => {
        word.addEventListener('click', function() {
            if (this.dataset.target === "true") {
                ...
                this.classList.add('bg-green-500', 'text-white', 'shadow-[0_0_15px_rgba(34,197,94,0.6)]');
                this.dataset.found = "true";
            } else {
                this.classList.add('bg-red-500/80', 'text-white');
                setTimeout(() => this.classList.remove('bg-red-500/80', 'text-white'), 1000);
            }
        });
    });
}
...
function createDraggableChip(text, expected, index) {
    ...
    chip.className = "draggable-chip cursor-grab text-center text-gray-900 text-lg font-bold shadow-md transition-all flex items-center justify-center mb-4";
...
function handleDrop(e, dropzone) {
    e.preventDefault();
    dropzone.classList.remove('border-pink-400', 'bg-pink-500/10', 'bg-pink-500/20');
    if (draggedItem) {
        ...
        if (dropzone.dataset.mode === 'flowing') {
            draggedItem.classList.remove('mb-4', 'shadow-md');
            draggedItem.classList.add('m-0', 'px-4', 'py-1', 'min-h-[35px]', 'text-base');
            draggedItem.style.borderRadius = '9999px'; // Only pill-shaped inside the sentence!
        } else {
            draggedItem.classList.remove('mb-4', 'shadow-md');
            draggedItem.classList.add('w-full', 'h-full', 'm-0');
            draggedItem.style.borderRadius = '8px'; 
        }
        ...
        if (dropzone.dataset.parentIndex) {
            ...
            if (block) {
                block.style.backgroundColor = draggedItem.dataset.color; block.style.color = "#111827";
...
function resetChipAppearance(chip) {
    chip.classList.remove('w-full', 'h-full', 'm-0', 'px-4', 'py-1', 'min-h-[35px]', 'text-base');
    chip.classList.add('mb-4', 'shadow-md');
    chip.style.backgroundColor = chip.dataset.color;
    chip.style.borderRadius = "12px"; // Back to soft rectangle when returned to left panel!
...
function checkAnswers(activity) {
    ...
    let allCorrect = true; let detailsArr = [];
    ...
    if (isDetective) {
        ...
        dropzones.forEach(dz => {
            ...
            if (dz.children.length > 0) {
                ...
                if (chip.dataset.expected === expected) {
                    chip.style.backgroundColor = "#4ade80"; // Correct green
                    detailsArr.push("Correct");
                } else {
                    allCorrect = false; detailsArr.push("Incorrect");
                    resetChipAppearance(chip); document.getElementById("left-panel").appendChild(chip);
                    dz.style.border = '';
                    if (dz.dataset.parentIndex) {
                        let block = document.querySelector(`.text-block[data-index="${dz.dataset.parentIndex}"]`);
    ...
    Papa.parse(TRACKING_CSV_URL, {
        ...
        complete: function(results) {
            ...
            data.forEach(row => {
                ...
                let statusBadge = row.Status === 'Completed' ? '<span class="bg-green-500/20 text-green-300 px-3 py-1 rounded-full text-xs font-bold border border-green-500/50">Completed</span>' 
                                : '<span class="bg-yellow-500/20 text-yellow-300 px-3 py-1 rounded-full text-xs font-bold border border-yellow-500/50">Attempting</span>';
