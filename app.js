...
style.textContent = `
    ...
    .draggable-chip {
        ...
        color: #000000 !important; /* Forces text to be dark on pastel chips */
    ...
    /* Makes the dashed borders disappear when a chip drops in */
    .drop-zone.filled {
...
function initStudentAuth() {
    ...
    app.innerHTML = `
        <div class="max-w-md mx-auto bg-white rounded-xl shadow-lg p-8 mt-20 text-center relative z-10">
            ...
            <!-- TEACHER DASHBOARD LINK RESTORED HERE -->
            <div class="mt-6 pt-4 border-t border-gray-100">
                <a href="?role=teacher" class="text-sm text-gray-500 hover:text-blue-600 flex justify-center items-center gap-2">
...
function renderChoices() {
    ...
    choices.forEach((choice, index) => {
        ...
        div.className = 'draggable-chip p-3 rounded-lg shadow font-bold text-center border-2 border-transparent';
...
function renderGapFill() {
    ...
    if (isCategorisation) {
        ...
        parts.forEach((part, index) => {
            ...
            // Fixed alignment: Changed inline-block w-32 to inline-flex min-w-[8rem]
            let html = part.Text.replace('___', `<span class="drop-zone inline-flex w-auto min-w-[8rem] h-[2.5rem] border-2 border-dashed border-gray-400 bg-white align-middle mx-2 justify-center items-center rounded" data-expected="${part.Label}" data-index="${index}"></span>`);
        ...
        parts.forEach((part, index) => {
            ...
            // Fixed alignment: Changed inline-block w-32 to inline-flex min-w-[8rem]
            let replacedText = partText.replace('___', `<span class="drop-zone inline-flex w-auto min-w-[8rem] h-[2.5rem] border-2 border-dashed border-gray-400 bg-white align-middle mx-2 justify-center items-center rounded" data-expected="${part.Label}" data-index="${index}"></span>`);
            if (part.Hint) {
...
function updateGapColors() {
    // Hide dashed lines when drop zone is filled
    ...
    // Handle background color bleeding
